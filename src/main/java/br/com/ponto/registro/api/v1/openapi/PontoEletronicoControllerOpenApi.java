...
public interface PontoEletronicoControllerOpenApi {
	...
	@Operation(summary = "Recupera lançamentos de ponto por usuário e data", responses = {
			@ApiResponse(responseCode = "200", description = "Recurso retornado com sucesso.", content = {
					@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = PontoEletronicoDTO.class)) }),
			@ApiResponse(responseCode = "400", description = "Requisição inválida.", content = {
					@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErroDTO.class)) }),
			@ApiResponse(responseCode = "401", description = "Você não tem autorização para executar esta operação.", content = {
					@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErroDTO.class)) }),
	...
	public ResponseEntity<List<PontoEletronicoDTO>> get(
			@Parameter(description = "Representação de data de lançamentos a serem buscados", required = true) final LocalDate data);
}